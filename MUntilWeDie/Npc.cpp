...
void Npc::chageImg(eStat changeStat, eDirection direction)
{
	...
	{
		cout <<((direction == eDirection::Left) ? "왼쪽" : "오른쪽")  << "  ";
		cout <<((mCurDirection == eDirection::Left) ? "왼쪽" : "오른쪽") << endl;
		mCurAni->ChangeCurImage(changeStat, direction);
	}
	else {
		cout << "n" << ((direction == eDirection::Left) ? "왼쪽" : "오른쪽") << "  " << "/" ;
		cout << "n" << ((mCurDirection == eDirection::Left) ? "왼쪽" : "오른쪽") << "  " << endl ;
	}
}
